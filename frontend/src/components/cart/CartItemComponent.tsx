...
type CartItemComponentProps = CartItem & {
  changeCart: (req: ChangeCartRequest) => void
  email: string;
};
...
const CartItemComponent = ({
  ...
  const handleClickQty = (amount: number) => {
    // 여기서 dispatch(postChangeCartAsync({ pno, qty: amount })) 같은 거 들어가면 됨
    const req: ChangeCartRequest = {
        email, cino, pno, qty:qty+amount
    }
    changeCart(req);
  };
      ...
      <div className="w-full border-2">
        ...
        <div className="justify-center p-2 text-xl">
          ...
          <div className="flex text-white font-bold p-2 justify-center">
            <button
              className="m-1 p-1 text-xl bg-red-500 w-8 rounded-lg"
